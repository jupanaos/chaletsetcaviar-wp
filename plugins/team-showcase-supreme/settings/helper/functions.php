...
function wpm_6310_link_css_js()
{
	...
	wp_enqueue_script('wpm-admin-js', wpm_6310_plugin_dir_url . 'assets/js/wpm-admin-script.js', array('jquery'));
}
...
function wpm_6310_toggle_service(){
	...
	if($wpm_6310_service_ads != ''){
		$wpdb->query("UPDATE {$wpdb->prefix}options 
			...
			where option_name = 'wpm_6310_service_ads'");
	 }
	 else{
		$wpdb->query("DELETE FROM {$wpdb->prefix}options where option_name='wpm_6310_service_ads'");
		$wpdb->query("INSERT INTO {$wpdb->prefix}options(option_name, option_value) VALUES ('wpm_6310_service_ads', '{$date}')");
	 }
	wp_die();
}

function wpm_6310_service_ads_status(){
	global $wpdb;
	$date = date('Y-m-d');
	$data = $wpdb->get_row("SELECT * FROM {$wpdb->prefix}options WHERE option_name = 'wpm_6310_service_ads' and option_value < '$date'");
	return $data ? '' : 1;
}
