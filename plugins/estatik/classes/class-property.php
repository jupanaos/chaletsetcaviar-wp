...
class Es_Property extends Es_Post
{
    ...
    public function save_address_components( $data ) {
        if ( ! empty( $data ) ) {
			global $wpdb;
			$wpdb->query( "DELETE FROM {$wpdb->postmeta} WHERE post_id='{$this->getID()}' AND meta_key LIKE '_address_component_%'" );
            foreach ( $data as $item ) {
                $component_id = ES_Address_Components::save_component( $item );
                if ( ! empty( $component_id ) ) {
					if ( ! empty( $item->types ) ) {
						foreach ( $item->types as $type ) {
							ES_Address_Components::save_property_component( $this->getID(), $component_id, $type );
						}
					}
                }
            }
    ...
    public function save_fields( $data ) {
        if ( ! empty( $data ) ) {
            ...
            if ( ! empty( $data['gallery'] ) ) {
				global $wpdb;
				$wpdb->update( $wpdb->posts, array( 'post_parent' => '' ), array( 'post_parent' => $this->getID(), 'post_type' => 'attachment' ) );
